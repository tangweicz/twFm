...
parse = OptionParser()
parse.add_option('-t', '--table', action='store', type="string", dest='tablename', help="tablename")
parse.add_option("-c", "--createController", action="store_true", default=False,  dest="doCreateController", help="doCreateController")
parse.add_option("-m", "--createModel", action="store_true", default=False,  dest="doCreateModel", help="doCreateModel")
options, args = parse.parse_args()
tablename = options.tablename#这儿拿到要参与脚本运行的表名，如果没有输入，则所有表参与生成
doCreateController = options.doCreateController
doCreateModel = options.doCreateModel
...
if not doCreateController and not doCreateModel:
    totalNum = 0
else:
    if tablename == None:
...
if totalNum >= 1:
    ...
    for row in results:#循环出所有的表名
        ...
        if tablename == None or everyTableName == tablename:
            ...
            if nowNum == totalNum:
                sys.stdout.write("正在生成文件" + "-" * precent + "" + str(precent) + "%    [" + str(nowNum) + "/" + str(totalNum) + "]" + "\n")
            else:
